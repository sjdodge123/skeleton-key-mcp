...
const SERVER_INSTRUCTIONS = [
  ...
  "   - Password / API token: `request_credential` returns a one-time, TOTP-gated web link the user opens to type the secret straight into the vault; poll `credential_request_status` until it's 'fulfilled'. Pass `fields` to collect a whole set of values on one link (e.g. an app's env secrets), stored as fields of one vault item.",
