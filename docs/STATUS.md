...
- **`feat/proxmox-connector` (#45)** — bespoke Proxmox VE connector (gap #2): API-token/ticket auth (incl. the safe `request_credential` handoff), HTTPS-enforced, read nodes/guests/tasks + task logs, and an approval-gated `guest_power` (start/shutdown/reboot/stop; the guest's name + node are re-verified against the live lookup before acting). Adversarially reviewed (5 rounds), awaiting owner merge.

_(The Home Assistant connector, #44 — which also fixes the long-standing double-encoded-POST `400` — is now **merged to `main`**; see gap #2.)_

## Known gaps / good next tasks

Roughly in priority order — pick up any of these:

0. ~~**Verify bearer while locked (#20)**~~ **DONE** — `src/config/bearer-hash.ts` persists a lock-independent SHA-256 of the bearer; `AppState.verifyBearer` checks it while locked and `ensureBearerHash` backfills it, so a valid legacy bearer is admitted to the banner-only `get_started` instead of 401'd after a restart.
...
2. **Bespoke connectors** (the biggest value). `ssh`/`http` plus **`portainer`** (Docker mgmt incl. `update_stack`), **`unifi`** (surgical gateway/network writes), **`home-assistant`** (merged #44 — REST + long-lived token; fixes the double-encoded-POST `400`), and **`proxmox`** (PR #45 — VE REST, API token/ticket, guest power control). Discovery maps `portainer`/`unifi`/`home-assistant`/`proxmox` detections to their real connectors; `synology`, `pihole` still fall back to `http`. Next suggested: **Synology DSM** (multi-host), **Pi-hole** (stats/blocklists). Each is a `Connector` in `src/connectors/`.
