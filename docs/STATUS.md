...
1. **Bespoke connectors** (the biggest value). `ssh`/`http` plus **`portainer`** (done — Docker mgmt incl. `update_stack` to redeploy a stack from an edited compose). Discovery maps `portainer` detections to the real connector now; `synology`, `proxmox`, `unifi`, `home-assistant`, `pihole` still fall back to `http`. Next suggested: **Home Assistant** (REST + long-lived token), **Proxmox** (API token). Each is a `Connector` in `src/connectors/`.
...
4. **Operational polish** — consider a scan progress indicator (the fingerprinting scan is slower than the old port scan).
