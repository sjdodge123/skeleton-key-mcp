...
- `src/web/auth.ts` — `mcpAuth`: setup-complete gate, then accepts a valid OAuth access token **or** the legacy static bearer; 401s carry the RFC 9728 `WWW-Authenticate` discovery hint. A locked vault does not block authenticated sessions — tool calls degrade to actionable "unlock at `<url>`" errors (see the CallTool locked gate in `src/mcp/server.ts`) so clients never see an opaque 503 after a container restart.
