...
- **Blast-radius limits:** SSH allow/deny command lists (deny `rm -rf`, `mkfs`, `dd`, etc.). Pelican ships **no** server-delete tool, its two API keys are matched to their half by required `papp_`/`pacc_` prefix (never falling back to the other), `create_server` fails closed without the target's dedicated `ownerUserId` so Claude's servers can never be owned by the admin's account, and a panel key is refused over plain http to anything but an RFC1918 host. UniFi execute tools are scoped to gateway feature toggles / IPv6 / remote logging / port-forward CRUD / force-provision (each a surgical read-modify-write that reports prior state so it can be reverted); remote-logging egress is guarded to private LAN IPv4; **port-forwards** — the only tools that open an inbound hole from the internet — are guarded *before the request is sent* by three hard checks: the destination must be a private LAN IPv4, it may not be a control-plane host (the gateway, Portainer/the container host, Vaultwarden, Skeleton Key itself — derived from the live target registry, never hardcoded), and neither the WAN nor the destination port range may touch an admin port (22, 443, 8006, 8787, 9000, …) whatever the host; a context that cannot supply the deny-list is refused rather than defaulted to empty; destructive DSM endpoints stay out of v1.
