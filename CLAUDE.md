...
- **Setup gating:** the MCP endpoint returns 503 until `data/setup-complete.json` exists (`src/web/auth.ts`). A locked vault (post-restart) does **not** 503 authenticated clients: sessions still connect and tool calls short-circuit with "unlock at `<admin URL>`" guidance (`get_started`/`list_targets`/`network_scan` keep working — `availableWhenLocked`). Setup-mutating API routes fail closed once setup is complete.
