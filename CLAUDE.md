...
Connector-per-integration behind a single MCP tool registry. Each connector (implemented: `ssh`, `http`, `portainer`, `unifi`, `home-assistant`, `proxmox`, `pelican`; planned: `synology`, dedicated `pihole`/`network` — Pi-hole is reached today as an `ssh` target and the LAN via `network_scan`) exposes tools split into two tiers:
...
Connectors fetch credentials from the Vaultwarden secrets client at call time (held in memory only); they never read a secrets store directly. The **execute set is Portainer, SSH, Home Assistant, Proxmox, and UniFi** (UniFi does surgical, revertable network changes: DPI/UPnP/offload/GeoIP toggles, per-network IPv6, off-box remote logging, and force-provision) — other connectors are read-only until asked otherwise. Everything a tool does lands in an append-only audit table. The Synology connector is **multi-host** (two DiskStations + a VirtualDSM).
...
Config comes from env (see `src/config/paths.ts`): `SKELETON_KEY_DATA_DIR` (runtime state), `SKELETON_KEY_PORT`, `SKELETON_KEY_BIND_HOST`, `SKELETON_KEY_UNLOCK_KEY_FILE` (path of the boot auto-unlock key file, default `/run/secrets/skeleton-key/unlock-key` — the key itself is a *random* value written there by the web UI when the admin enables auto-unlock; a keyslot on the bootstrap store, so the master passphrase never touches disk. Must live in a host mount outside the data volume), `SKELETON_KEY_PASSPHRASE` / `SKELETON_KEY_PASSPHRASE_FILE` (**deprecated** passphrase-in-environment boot unlock — still honored with a boot warning; the `_FILE` form takes precedence and fails closed to manual web-UI unlock on an unreadable file), `SKELETON_KEY_DISABLE_EXECUTE=1` (kill-switch for all execute tools), `SKELETON_KEY_CREDENTIAL_TTL_MINUTES` (default lifetime of a `request_credential` link, 30; clamped 5–240; per-request `ttlMinutes` overrides), `SKELETON_KEY_PUBLIC_URL` (absolute base URL for user-facing links; **auto-detected** from the LAN on first boot and persisted to `data/public-url`, so the env var is only needed to override — see `src/config/public-url.ts`; its scheme is migrated in place when TLS flips on/off), `SKELETON_KEY_TLS` (**LAN TLS, on by default** — a self-signed cert is generated into `data/tls/` on first boot and re-issued near expiry or when the public-URL host changes; `off` opts out; generation failure falls back to plain HTTP with a warning so an auto-upgraded container always boots — see `src/web/tls.ts`), `SKELETON_KEY_TLS_CERT_FILE`/`SKELETON_KEY_TLS_KEY_FILE` (user-mounted PEM pair, takes precedence over the self-managed cert; must be set together and fails the boot loudly if unreadable).
...
- **Two tool kinds:** per-target connector tools (above) and **global tools** (`src/mcp/builtin-tools.ts`, `buildGlobalTools`) that operate on the vault/registry themselves (`network_scan`, `vault_generate_ssh_key`, `vault_store_login`, `vault_list_credentials`, `vault_validate_ssh`, `register_target`, `update_target`, `vault_delete_credential`, `request_credential`, `credential_request_status`, `list_targets`, `form_skeleton` — an encrypted disaster-recovery config snapshot of every registered target, run as a background job (`src/snapshots/jobs.ts`; a second call joins the running job) — and `skeleton_status` to poll it). `resolveTools` normalizes both into a `ResolvedTool` with a bound `invoke(input)` and a `targetName` (null for global); the MCP handler treats them uniformly. Global tools enable conversational onboarding — Claude maps the LAN, generates/stores keys, and registers targets in chat. Writing to the vault uses `bw create item` via `VaultwardenClient.createLoginItem` (needs connectivity + `ssh-keygen` in the image).
- **Secrets never hit the registry:** `targets.yaml` stores only a `credentialRef` (a Vaultwarden item name) plus the pinned `credentialId` (the item's immutable UUID, resolved at register/update time); the actual credential is fetched in-memory at call time via `AppState.credentialFor` — by id when pinned (so a rename can't serve a stale value), falling back to the name and re-pinning. Trashed vault items never satisfy a ref. Deploy-time reads (Portainer `secretEnv`) pass `fresh: true` to re-sync the offline cache first.
- **Never ask for secrets in chat:** to onboard a host that needs a password/token, use `request_credential` — it mints a one-time, TOTP-gated web link (`src/web/credential-routes.ts`) where the user types the secret straight into the vault. When the expected shape is known, pass per-field `pattern`/`minLength`/`hint` (the form refuses a wrong value) and a `verify` probe (tested live before storing; the probe host is shown to the human because the secret is sent there); use `overwrite: true` to refill an existing item rather than minting new names; `/admin/credentials` lists open links. Never weaken these to "just accept it" — the Discord client-secret-vs-bot-token mix-up cost a whole evening. Fingerprints (`src/lib/fingerprint.ts`, keyed) are the sanctioned way to compare a vault value with a deployed one; never echo values. Secrets must never transit the chat/MCP channel or the model context. User-facing links that ask for a secret (this + unlock guidance) use `SKELETON_KEY_PUBLIC_URL` only, never the request `Host` header (anti-phishing).
